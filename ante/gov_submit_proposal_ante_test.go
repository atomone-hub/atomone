...
// TestGovSubmitProposalDecoratorAuthz checks that authz.MsgExec wrappers are
// expanded when inspecting proposal messages.
func TestGovSubmitProposalDecoratorAuthz(t *testing.T) {
	atomoneApp := helpers.Setup(t)
	ctx := atomoneApp.NewUncachedContext(true, tmproto.Header{})
	cdc := atomoneApp.AppCodec()
	decorator := ante.NewGovSubmitProposalDecorator(cdc, atomoneApp.CoreDaosKeeper)
	...
	// mustMsgExec wraps msgs in an authz.MsgExec.
	mustMsgExec := func(grantee sdk.AccAddress, msgs ...sdk.Msg) *authz.MsgExec {
	...
	tests := []struct {
		...
		{
			name: "authz MsgExec inside proposal bundles oversight change — rejected",
			msgs: []sdk.Msg{mustNewAtomOneSubmitProposal(t, []sdk.Msg{
				mustMsgExec(addrs[2], updateParamsChanging, otherMsg),
			}, proposer)},
			expectPass: false,
		},
		{
			name: "authz MsgExec for oversight change alongside other proposal msg — rejected",
			msgs: []sdk.Msg{mustNewAtomOneSubmitProposal(t, []sdk.Msg{
				mustMsgExec(addrs[2], updateParamsChanging),
				otherMsg,
			}, proposer)},
			expectPass: false,
		},
		{
			name: "authz MsgExec with single oversight change — allowed",
			msgs: []sdk.Msg{mustNewAtomOneSubmitProposal(t, []sdk.Msg{
				mustMsgExec(addrs[2], updateParamsChanging),
			}, proposer)},
			expectPass: true,
		...
		{
			name: "authz MsgExec wrapping submit-proposal that bundles — rejected",
			msgs: []sdk.Msg{mustMsgExec(addrs[2],
				mustNewAtomOneSubmitProposal(t, []sdk.Msg{updateParamsChanging, otherMsg}, proposer),
			)},
			expectPass: false,
		},
		{
			name: "authz MsgExec wrapping valid single-message submit-proposal — allowed",
			msgs: []sdk.Msg{mustMsgExec(addrs[2],
				mustNewAtomOneSubmitProposal(t, []sdk.Msg{updateParamsChanging}, proposer),
			)},
			expectPass: true,
		},
	}
