...
func (s *IntegrationTestSuite) testCoreDAOs() {
	...
	s.Run("coredaos parameter change", func() {
		...
		params, err := s.queryCoreDAOsParams(chainAAPIEndpoint)
		...
		newParams, err := s.queryCoreDAOsParams(chainAAPIEndpoint)
		s.Require().NoError(err)
	...
	s.Run("coredaos annotation", func() {
		...
		proposal, err := s.queryGovV1Proposal(chainAAPIEndpoint, proposalCounter)
		s.Require().NoError(err)
	...
	s.Run("coredaos extend voting period", func() {
		...
		proposalID := s.submitVotingPeriodLawProposal(s.chainA)
		proposalBeforeExtension, err := s.queryGovV1Proposal(chainAAPIEndpoint, proposalID)
		s.Require().NoError(err)
		...
		s.executeMultiSigTxCommand(s.chainA, atomoneCommand, valIdx, steeringDAOAccount, false)
		proposalAfterExtension, err := s.queryGovV1Proposal(chainAAPIEndpoint, proposalID)
		s.Require().NoError(err)
	...
	s.Run("coredaos endorse", func() {
		...
		proposalID := s.submitVotingPeriodLawProposal(s.chainA)
		proposalBeforeEndorsement, err := s.queryGovV1Proposal(chainAAPIEndpoint, proposalID)
		s.Require().NoError(err)
		...
		s.executeMultiSigTxCommand(s.chainA, atomoneCommand, valIdx, steeringDAOAccount, false)
		proposalAfterEndorsement, err := s.queryGovV1Proposal(chainAAPIEndpoint, proposalID)
		s.Require().NoError(err)
	...
	s.Run("coredaos veto", func() {
		...
		proposalID := s.submitVotingPeriodLawProposal(s.chainA)
		proposalBeforeVeto, err := s.queryGovV1Proposal(chainAAPIEndpoint, proposalID)
		s.Require().NoError(err)
		...
		proposalAfterVeto, err := s.queryGovV1Proposal(chainAAPIEndpoint, proposalID)
		s.Require().NoError(err)
