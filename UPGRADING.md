...
AtomOne v4 build requires Go compiler version 1.26.4. If you already have go
installed but with another version, you can install go1.26.4 with the following
command:
...
```
$ GOROOT=$(go1.26.4 env GOROOT) PATH=$GOROOT/bin:$PATH make build
```
...
network. The new version requires `go v1.26.4` to build successfully.
