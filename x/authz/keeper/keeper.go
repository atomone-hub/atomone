...
func (k Keeper) DispatchActions(ctx sdk.Context, grantee sdk.AccAddress, msgs []sdk.Msg) ([][]byte, error) {
	...
	for i, msg := range msgs {
		...
		if !granter.Equals(grantee) {
			...
			grant, found := k.getGrant(ctx, skey)
			if !found {
				return nil, sdkerrors.Wrapf(authz.ErrNoAuthorizationFound, "failed to update grant with key %s", string(skey)) //nolint: staticcheck
			}
		...
		if err != nil {
			return nil, sdkerrors.Wrapf(err, "failed to execute message; message %v", msg) //nolint: staticcheck
		}
...
func (k Keeper) DeleteGrant(ctx sdk.Context, grantee sdk.AccAddress, granter sdk.AccAddress, msgType string) error {
	...
	grant, found := k.getGrant(ctx, skey)
	if !found {
		return sdkerrors.Wrapf(authz.ErrNoAuthorizationFound, "failed to delete grant with key %s", string(skey)) //nolint: staticcheck
	}
...
func (k Keeper) insertIntoGrantQueue(ctx sdk.Context, granter, grantee sdk.AccAddress, msgType string, expiration time.Time) error {
	...
	if len(queueItems.MsgTypeUrls) == 0 {
		...
		if err = k.setGrantQueueItem(ctx, expiration, granter, grantee, queueItems); err != nil {
			return err
		}
	}
...
func (k Keeper) removeFromGrantQueue(ctx sdk.Context, grantKey []byte, granter, grantee sdk.AccAddress, expiration time.Time) error {
	...
	bz := store.Get(key)
	if bz == nil {
		return sdkerrors.Wrap(authz.ErrNoGrantKeyFound, "can't remove grant from the expire queue, grant key not found") //nolint: staticcheck
	}
