...
// FeeDecorator implements sdk.AnteDecorator and ensures that uphoton is the
// only fee token, except for some specific messages for which the type URLs
// are stored in the module parameters.
...
func (fd FeeDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (sdk.Context, error) {
	...
	if isTxFeeExcepted(tx, fd.keeper.GetParams(ctx).TxFeeExceptions) {
