...
func (app *BaseApp) Query(req abci.RequestQuery) (res abci.ResponseQuery) {
	...
	defer func() {
		if r := recover(); r != nil {
			res = sdkerrors.QueryResult(sdkerrors.Wrapf(sdkerrors.ErrPanic, "%v", r), app.trace) //nolint: staticcheck
		}
	}()
	...
	if req.Path == "/atomone.tx.v1beta1.Service/BroadcastTx" {
		return sdkerrors.QueryResult(sdkerrors.Wrap(sdkerrors.ErrInvalidRequest, "can't route a broadcast tx message"), app.trace) //nolint: staticcheck
	}
	...
	if len(path) == 0 {
		return sdkerrors.QueryResult(sdkerrors.Wrap(sdkerrors.ErrUnknownRequest, "no query path provided"), app.trace) //nolint: staticcheck
	}
	...
	return sdkerrors.QueryResult(sdkerrors.Wrap(sdkerrors.ErrUnknownRequest, "unknown query path"), app.trace) //nolint: staticcheck
}
...
func gRPCErrorToSDKError(err error) error {
	...
	if !ok {
		return sdkerrors.Wrap(sdkerrors.ErrInvalidRequest, err.Error()) //nolint: staticcheck
	}

	switch status.Code() {
	case codes.NotFound:
		return sdkerrors.Wrap(sdkerrors.ErrKeyNotFound, err.Error()) //nolint: staticcheck
	case codes.InvalidArgument:
		return sdkerrors.Wrap(sdkerrors.ErrInvalidRequest, err.Error()) //nolint: staticcheck
	case codes.FailedPrecondition:
		return sdkerrors.Wrap(sdkerrors.ErrInvalidRequest, err.Error()) //nolint: staticcheck
	case codes.Unauthenticated:
		return sdkerrors.Wrap(sdkerrors.ErrUnauthorized, err.Error()) //nolint: staticcheck
	default:
		return sdkerrors.Wrap(sdkerrors.ErrUnknownRequest, err.Error()) //nolint: staticcheck
	}
}
...
func (app *BaseApp) CreateQueryContext(height int64, prove bool) (sdk.Context, error) {
	...
	if lastBlockHeight == 0 {
		return sdk.Context{}, sdkerrors.Wrapf(sdkerrors.ErrInvalidHeight, "%s is not ready; please wait for first block", app.Name()) //nolint: staticcheck
	}

	if height > lastBlockHeight {
		return sdk.Context{},
			sdkerrors.Wrap( //nolint: staticcheck
	...
	if err != nil {
		return sdk.Context{},
			sdkerrors.Wrapf( //nolint: staticcheck
				sdkerrors.ErrInvalidRequest,
...
func handleQueryApp(app *BaseApp, path []string, req abci.RequestQuery) abci.ResponseQuery {
	if len(path) >= 2 {
		...
		case "simulate":
			...
			if err != nil {
				return sdkerrors.QueryResult(sdkerrors.Wrap(err, "failed to simulate tx"), app.trace) //nolint: staticcheck
			}
			...
			if err != nil {
				return sdkerrors.QueryResult(sdkerrors.Wrap(err, "failed to JSON encode simulation response"), app.trace) //nolint: staticcheck
			}
		...
		default:
			return sdkerrors.QueryResult(sdkerrors.Wrapf(sdkerrors.ErrUnknownRequest, "unknown query: %s", path), app.trace) //nolint: staticcheck
		}
	}
...
func handleQueryStore(app *BaseApp, path []string, req abci.RequestQuery) abci.ResponseQuery {
	...
	if !ok {
		return sdkerrors.QueryResult(sdkerrors.Wrap(sdkerrors.ErrUnknownRequest, "multistore doesn't support queries"), app.trace) //nolint: staticcheck
	}
...
func handleQueryP2P(app *BaseApp, path []string) abci.ResponseQuery {
	...
	default:
		resp = sdkerrors.QueryResult(sdkerrors.Wrap(sdkerrors.ErrUnknownRequest, "expected second parameter to be 'filter'"), app.trace) //nolint: staticcheck
	}
