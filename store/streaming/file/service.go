...
func writeLengthPrefixedFile(path string, data []byte, fsync bool) (err error) {
	...
	if err != nil {
		return sdkerrors.Wrapf(err, "open file failed: %s", path) //nolint: staticcheck
	}

	defer func() {
		// avoid overriding the real error with file close error
		if err1 := f.Close(); err1 != nil && err == nil {
			err = sdkerrors.Wrapf(err, "close file failed: %s", path) //nolint: staticcheck
		}
	}()

	_, err = f.Write(sdk.Uint64ToBigEndian(uint64(len(data))))
	if err != nil {
		return sdkerrors.Wrapf(err, "write length prefix failed: %s", path) //nolint: staticcheck
	}

	_, err = f.Write(data)
	if err != nil {
		return sdkerrors.Wrapf(err, "write block data failed: %s", path) //nolint: staticcheck
	}

	if fsync {
		err = f.Sync()
		if err != nil {
			return sdkerrors.Wrapf(err, "fsync failed: %s", path) //nolint: staticcheck
		}
	}
